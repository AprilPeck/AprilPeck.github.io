---
title: "Models of Neighborhood Change in San Francisco"
output: 
...
github.url <- "https://aprilpeck.github.io/CPP529-Final-Dashboard/sfca_dorling.geojson"
sfca <- geojson_read( x=github.url,  what="sp", stringsAsFactors = FALSE )

# reproject the map 
sfca2 <- spTransform( sfca, CRS("+init=epsg:3395") )

# convert the sp map format to 
# an sf (simple features) format:
# ggmap requires the sf format
sfca.sf <- st_as_sf( sfca2 )

# separate out the data frame from the map
d <- as.data.frame( sfca.sf )
...
these.names <- c("Percent white, non-Hispanic", 
...
temp.names <- paste0( "Variable ", these.names )
...
ggplot( get_data() ) +
    ...
    xlim( xmin = -13645063, xmax = -13592179 ) + 
    ylim( ymin = 4470284, ymax = 4529237 )
...
bb <- st_bbox( c( xmin =  -13645063, xmax = -13592179, 
                  ymax = 4529237, ymin = 4470284 ), 
               crs = st_crs("+init=epsg:3395"))

# ADD YOUR CUSTOM LABELS TO THE CLUSTERS 

sfca2$cluster[ sfca2$cluster == "1" ] <- "White, wealthy, educated families"
sfca2$cluster[ sfca2$cluster == "2" ] <- "Foreign-born, middle working-class families"
sfca2$cluster[ sfca2$cluster == "3" ] <- "Poor, black, single"
sfca2$cluster[ sfca2$cluster == "4" ] <- "Poor, hispanic, single-parent"
sfca2$cluster[ sfca2$cluster == "5" ] <- "White, middle-age, wealthy, single, renters"
...
x <- get_variable_x() %>% unlist() %>% as.numeric() %>% na.omit()
